...
class SlicerApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("NeuroXAI Viewer")
    ...
    def _serialize_measurements(self):
        ...
        for measure in self.measurements:
            measure_data = {
                ...
                'slice': measure['slice'],
                'color': measure['color'],
                'type': measure.get('type', 'distance')  # Default to distance for backward compatibility
            }
            
            if measure.get('type') == 'area':
                measure_data.update({
                    'points': measure['points'],
                    'area_pixels': measure['area_pixels'],
                    'area_mm2': measure['area_mm2']
                })
            else:
                measure_data.update({
                    'p1': measure['p1'],
    ...
    def _deserialize_measurements(self, serialized_measurements):
        ...
        for measure_data in serialized_measurements:
            measure = {
                ...
                'type': measure_data.get('type', 'distance')
            }
            
            if measure_data.get('type') == 'area':
                measure.update({
                    'points': measure_data['points'],
                    'area_pixels': measure_data['area_pixels'],
                    'area_mm2': measure_data['area_mm2']
                })
                if measure['id'] > max_id:
                    max_id = measure['id']
                    self.last_area = measure
            else:
                measure.update({
    ...
    def _create_menu(self):
        ...
        tools_menu.add_checkbutton(label="Distance Measurements", variable=self.measurement_mode)
        tools_menu.add_checkbutton(label="Area Measurements", variable=self.area_mode)  # NEW AREA MENU ITEM
    ...
    def _on_area_toggle(self, *args):
        ...
        if self.area_mode.get():
            ...
            self.temp_area_points = None
            if not any([self.measurement_mode.get(), self.draw_mode.get(), 
    ...
    def _on_pan_toggle(self, *args):
        if self.pan_mode.get():
            ...
            if not any([self.measurement_mode.get(), self.draw_mode.get(), 
                    self.zoom_select_mode.get(), self.area_mode.get()]):
                self.status_label.config(text="Ready")
                self._reset_cursors()
    ...
    def _on_zoom_select_toggle(self, *args): 
        if self.zoom_select_mode.get():
            ...
            self.update_2d_views()
            if not any([self.measurement_mode.get(), self.draw_mode.get(), 
                    self.pan_mode.get(), self.area_mode.get()]):
                self.status_label.config(text="Ready")
                self._reset_cursors()
    ...
    def _draw_slice_measurements(self, ax, plane_idx, slice_idx):
        ...
        for measure in self.measurements:
            ...
            if measure.get('type') == 'area':
                ...
                ax.text(centroid_x, centroid_y, area_text, 
        ...
        if (self.temp_area_points and 
            ...
            points = np.array(self.temp_area_points['points'])
            if len(points) > 1:
                ax.plot(points[:, 0], points[:, 1], 
                ...
                ax.text(points[0, 0], points[0, 1], "Start", 
    ...
    def clear_measurements(self, confirm=True):      
        ...
        if confirm and not messagebox.askyesno("Confirm", "Clear all measurements (distance and area)?", parent=self.root):
            return
    ...
    def add_measurement_comment(self):
        """Fixed to handle both distance and area measurements without duplicates"""
        if not self.last_measurement and not self.last_area:
        ...
        if self.last_area and self.last_measurement:
            ...
            if self.last_area['id'] > self.last_measurement['id']:
                ...
                target_measurement = self.last_measurement
        elif self.last_area:
            target_measurement = self.last_area
        elif self.last_measurement:
            target_measurement = self.last_measurement
        
        if 'comment' in target_measurement:
            messagebox.showinfo("Info", "This measurement already has a comment.", parent=self.root)
            return
        
        measurement_type = "area" if target_measurement.get('type') == 'area' else "distance"
        comment = simpledialog.askstring("Input", 
                                    f"Enter comment for the last {measurement_type} measurement:", 
                                    parent=self.root)
        if comment:
    ...
    def _add_item_to_tree(self, tree, item_data, item_type):
        ...
        if item_data.get('type') == 'area':
            measurement_info = f"Area: {item_data['area_mm2']:.2f}mm²"
    ...
    def _handle_measurement_click(self, event, view):
        ...
        if self.temp_measure_point is None:
            ...
            if (self.temp_measure_point['plane'] == plane_idx and 
                ...
                self.measurement_id_counter = measurement_id
    ...
    def on_button_press(self, event, view):
        ...
        if self.area_mode.get() and event.button == 1:
            ...
            self.status_label.config(text="Area Mode: Drag to draw lasso, release to complete area measurement.")
            self.update_2d_views()
            return
    ...
    def on_mouse_move(self, event, view):
        ...
        if (self.mouse_state == STATE_DRAW and self.area_mode.get() and 
            ...
            if (self.temp_area_points['plane'] == plane_idx and 
                self.temp_area_points['slice'] == self.current_slices[plane_idx]):
    ...
    def _finalize_area_measurement(self, event, view):
        ...
        plane_idx = self.temp_area_points['plane']
        ...
        self.temp_area_points = None
        self.status_label.config(text=f"Area measurement created: {area_mm2:.2f} mm²")
        self.update_2d_views()
    ...
    def _increment_measurement_counters(self):
        ...
        for measure in self.measurements:
            if measure.get('id', 0) > max_existing_id:
                max_existing_id = measure['id']
        
        self.measurement_id_counter = max(self.measurement_id_counter, max_existing_id) + 1
    ...
    def _cancel_area_measurement(self, event=None):
        ...
        if self.temp_area_points:
            ...
            self.update_2d_views()
        elif self.temp_measure_point:
            self._cancel_measurement(event)
    ...
    def _on_measure_toggle(self, *args):
        if self.measurement_mode.get():
            ...
            self.temp_measure_point = None
            if not any([self.draw_mode.get(), self.zoom_select_mode.get(), 
                    self.pan_mode.get(), self.area_mode.get()]):
                self.status_label.config(text="Ready")
                self._reset_cursors()
        self.update_2d_views()
    ...
    def _on_draw_toggle(self, *args):
        if self.draw_mode.get():
            ...
            if not any([self.measurement_mode.get(), self.zoom_select_mode.get(), 
                    self.pan_mode.get(), self.area_mode.get()]):
                self.status_label.config(text="Ready")
                self._reset_cursors()
    ...
    def _reset_cursors(self):
        if not any([self.draw_mode.get(), self.measurement_mode.get(), 
                self.zoom_select_mode.get(), self.pan_mode.get(), self.area_mode.get()]):
    ...
    def undo_action(self):
        ...
        if last_action['type'] == 'draw':
            ...
            if self.last_area and self.last_area['id'] == area['id']:
                self.last_area = self._find_most_recent_area()
            ...
            for measure in self.measurements:
                if 'comment' in measure:
                    self._add_item_to_tree(self.measurements_tree, measure, "Measure")
            self.status_label.config(text="Undo: Restored all measurements")
    ...
    def redo_action(self):
        ...
        if redo_action['type'] == 'draw':
            ...
            if 'comment' in area:
                self._add_item_to_tree(self.measurements_tree, area, "Measure")
            self.last_area = area
            ...
            for item in self.measurements_tree.get_children():
                self.measurements_tree.delete(item)
            self.status_label.config(text="Redo: Cleared all measurements")
    ...
    def _find_most_recent_area(self):
        ...
        highest_id = -1
        for measure in self.measurements:
            if measure.get('type') == 'area' and measure.get('id', 0) > highest_id:
                highest_id = measure['id']
                most_recent = measure
        return most_recent
        ...
        if item_type == 'Drawing':
                    ...
                    for stroke in strokes:
                        if stroke.get('id') == item_num:
                            ...
                            if new_comment is not None and new_comment != old_comment:
                                ...
                                self._update_annotation_in_tree(tree, item_id, stroke)
            ...
            for measure in self.measurements:
                if measure.get('id') == item_num:
                    ...
                    measurement_type = "area" if measure.get('type') == 'area' else "distance"
                    new_comment = simpledialog.askstring(
                        f"Edit {measurement_type.title()} Comment", 
                        f"Edit {measurement_type} measurement comment:",
                        initialvalue=old_comment,
                        parent=self.root
                    )
                    if new_comment is not None and new_comment != old_comment:
                        ...
                        self._update_annotation_in_tree(tree, item_id, measure)
                        self.status_label.config(text=f"Updated {measurement_type} {item_num}")
                    return
    ...
    def _update_annotation_in_tree(self, tree, item_id, annotation_data):
        ...
        if annotation_data.get('type') == 'area':
            measurement_info = f"Area: {annotation_data['area_mm2']:.2f}mm²"
...
if __name__ == "__main__":
    ...
    try:
        ...
        loading = LoadingWindow("NeuroXAI", "Loading Neuroimaging Slice Viewer...")
